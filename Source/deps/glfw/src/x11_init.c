...
//
static Atom getSupportedAtom(
        Atom *supportedAtoms, unsigned long atomCount, const char *atomName
                            ) {
...
static GLFWbool initExtensions(void) {
    ...
    if (_glfw.x11.xkb.available) {
        ...
        if (XkbGetState(_glfw.x11.display, XkbUseCoreKbd, &state) == Success) {
            XkbSelectEventDetails(_glfw.x11.display, XkbUseCoreKbd, XkbStateNotify, XkbAllStateComponentsMask,
                                  XkbGroupStateMask);
