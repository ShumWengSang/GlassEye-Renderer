...
static void shellSurfaceHandlePing(
        void *data, struct wl_shell_surface *shellSurface, uint32_t serial
                                  ) {
    wl_shell_surface_pong(shellSurface, serial);
}

static void shellSurfaceHandleConfigure(
...
static void shellSurfaceHandlePopupDone(
...
static void createDecoration(
        _GLFWdecorationWayland *decoration, struct wl_surface *parent, struct wl_buffer *buffer, GLFWbool opaque, int x
...
static GLFWbool createSurface(
        _GLFWwindow *window, const _GLFWwndconfig *wndconfig
                             ) {
...
static void setFullscreen(
        _GLFWwindow *window, _GLFWmonitor *monitor, int refreshRate
                         ) {
...
static void setCursorImage(
        _GLFWwindow *window, _GLFWcursorWayland *cursorWayland
                          ) {
...
int _glfwPlatformCreateWindow(
        _GLFWwindow *window, const _GLFWwndconfig *wndconfig, const _GLFWctxconfig *ctxconfig
...
void _glfwPlatformSetWindowIcon(
        _GLFWwindow *window, int count, const GLFWimage *images
                               ) {
    _glfwInputError(GLFW_PLATFORM_ERROR,
...
void _glfwPlatformSetWindowSizeLimits(
        _GLFWwindow *window, int minwidth, int minheight, int maxwidth, int maxheight
                                     ) {
    if (_glfw.wl.wmBase) {
        if (window->wl.xdg.toplevel) {
...
void _glfwPlatformSetWindowAspectRatio(
        _GLFWwindow *window, int numer, int denom
                                      ) {
    // TODO: find out how to trigger a resize.
    // The actual limits are checked in the wl_shell_surface::configure handler.
}

void _glfwPlatformGetFramebufferSize(
...
void _glfwPlatformGetWindowFrameSize(
        _GLFWwindow *window, int *left, int *top, int *right, int *bottom
                                    ) {
...
void _glfwPlatformGetWindowContentScale(
        _GLFWwindow *window, float *xscale, float *yscale
                                       ) {
    if (xscale)
...
void _glfwPlatformSetWindowMonitor(
        _GLFWwindow *window, _GLFWmonitor *monitor, int xpos, int ypos, int width, int height, int refreshRate
                                  ) {
    if (monitor) {
...
int _glfwPlatformCreateCursor(
        _GLFWcursor *cursor, const GLFWimage *image, int xhot, int yhot
                             ) {
...
static void relativePointerHandleRelativeMotion(
        void *data, struct zwp_relative_pointer_v1 *pointer, uint32_t timeHi, uint32_t timeLo, wl_fixed_t dx
...
static void lockedPointerHandleLocked(
        void *data, struct zwp_locked_pointer_v1 *lockedPointer
                                     ) {
}
...
static void dataSourceHandleTarget(
        void *data, struct wl_data_source *dataSource, const char *mimeType
                                  ) {
...
static void dataSourceHandleSend(
        void *data, struct wl_data_source *dataSource, const char *mimeType, int fd
                                ) {
...
static void dataSourceHandleCancelled(
        void *data, struct wl_data_source *dataSource
                                     ) {
...
int _glfwPlatformGetPhysicalDevicePresentationSupport(
